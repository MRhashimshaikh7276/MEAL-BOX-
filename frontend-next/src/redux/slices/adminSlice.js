...
import { initializeSocket, disconnectSocket, joinAdminRoom, joinDeliveryRoom } from '../../services/socket'
...
const adminSlice = createSlice({
  ...
  reducers: {
    ...
    initializeDeliverySocketConnection: (state, action) => {
      const { token } = action.payload;
      const socket = initializeSocket(token);
      state.socket = socket;
      joinDeliveryRoom();
...
export const { setSocket, initializeSocketConnection, initializeDeliverySocketConnection, disconnectSocketConnection } = adminSlice.actions
