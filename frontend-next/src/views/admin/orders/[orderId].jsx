...
export default function AdminOrderDetails() {
    ...
    const [preparationTime, setPreparationTime] = useState(order?.preparationTime || 15)
    const [deliveryTime, setDeliveryTime] = useState(order?.deliveryBoyEstimatedTime || 20)
    ...
    const fetchOrderDetails = async () => {
        try {
            ...
            setPreparationTime(response.data.order.preparationTime || 15)
            setDeliveryTime(response.data.order.deliveryBoyEstimatedTime || 20)
        } catch (error) {
    ...
    const handleStatusUpdate = async (status, preparationMinutes, deliveryMinutes) => {
        setUpdating(true)
        try {
            const payload = { status }
            if (status === 'preparing' && preparationMinutes) {
                payload.preparationTime = parseInt(preparationMinutes)
            }
            if (status === 'out_for_delivery' && deliveryMinutes) {
                payload.deliveryTime = parseInt(deliveryMinutes)
            }
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    ...
                    {/* Timing Info */}
                    {(order.preparationTime || order.estimatedDeliveryTime) && (
                        <div className="card p-5">
                            <h2 className="font-display font-bold text-lg text-gray-900 dark:text-white mb-4">Timing</h2>
                            <div className="space-y-3">
                                ...
                                {order.deliveryBoyEstimatedTime && (
                                    <div className="flex justify-between">
                                        <span className="text-gray-500">Delivery Time:</span>
                                        <span className="font-medium">{order.deliveryBoyEstimatedTime} min</span>
                                    </div>
                                )}
                                {order.estimatedDeliveryTime && (
                                    <div className="flex justify-between font-bold pt-2 border-t border-gray-100 dark:border-gray-800">
                                        ...
                                        <span className="text-primary-500">{order.estimatedDeliveryTime} min</span>
                                    </div>
                                )}
                                {order.preparingStartedAt && (
                                    <div className="text-xs text-gray-400 mt-2">
                                        Started: {new Date(order.preparingStartedAt).toLocaleTimeString()}
                                    </div>
                                )}
                                {order.outForDeliveryAt && (
                                    <div className="text-xs text-gray-400">
                                        Out for Delivery: {new Date(order.outForDeliveryAt).toLocaleTimeString()}
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="card p-5">
                        ...
                        {order.orderStatus === 'pending' && (
                            <div className="mb-3">
                                <label className="text-xs text-gray-500 mb-1 block">Preparing Time (minutes)</label>
                                <input
                                    type="number"
                        ...
                        {order.orderStatus === 'preparing' && (
                            <div className="mb-3">
                                <label className="text-xs text-gray-500 mb-1 block">Delivery Time (minutes)</label>
                                <input
                                    type="number"
                                    value={deliveryTime}
                        ...
                        <select
                            ...
                            onChange={(e) => handleStatusUpdate(e.target.value, preparationTime, deliveryTime)}
                            disabled={updating || order.orderStatus === 'delivered' || order.orderStatus === 'cancelled'}
                            className="w-full input py-2.5"
                        >
                            {STATUS_OPTIONS.filter(opt => opt.value !== 'delivered').map((opt) => (
