...
export default function ForgotPasswordPage() {
  ...
  const onSubmitPassword = async (data) => {
    setLoading(true)
    try { await authAPI.resetPassword(token, data.password); toast.success('Password reset!'); window.location.href = '/login' }
  ...
  if (hasToken) {
    return (
      <div>
        <Link to="/login" className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 mb-6 transition-colors">
