...
// banner slides will be generated dynamically from combos and offers
// config.js (optional clean structure)
...
export default function HomePage() {
  ...
  // Load all search data for dropdown
  useEffect(() => {
  ...
  const handleSearchResultClick = (type, item) => {
    ...
    switch (type) {
      case 'category':
        navigate(`/menu?categoryId=${item._id}`)
        break
      case 'subcategory':
        navigate(`/menu?subcategoryId=${item._id}`)
        break
      case 'subsubcategory':
        navigate(`/menu?subsubcategoryId=${item._id}`)
        break
      case 'product':
  ...
  useEffect(() => {
    // whenever the banner list changes ensure the index is valid
    if (activeBanner >= banners.length) {
      setActiveBanner(0)
    }
    // update interval when number of banners changes
    if (banners.length > 0) {
      const interval = setInterval(
    ...
    // no banners, nothing to clean up
    return undefined
  }, [banners, activeBanner])

  const handleSearch = (e) => {
    e.preventDefault()
    if (search.trim()) navigate(`/menu?search=${search.trim()}`)
  }
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="px-4 py-6 space-y-8">
        {/* Mobile Search */}
        <form onSubmit={handleSearch} className="md:hidden" ref={searchInputRef}>
          ...
          {/* Search Dropdown */}
          {showSearchDropdown && (
            <div className="absolute top-72% left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 max-h-96 overflow-y-auto z-50">
                  ...
                  {searchResults.subcategories.length > 0 && (
                    <div className="py-2 border-t border-gray-100 dark:border-gray-700">
                      ...
                      {searchResults.subcategories.map(sub => (
                        ...
                        >
                          <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-100">
                            {sub.image && <img src={`${baseUrl}${sub.image}`} alt={sub.name} className="w-full h-full object-cover" />}
                          </div>
                  ...
                  {searchResults.subsubcategories.length > 0 && (
                    <div className=" border-t border-gray-100 dark:border-gray-700">
                      ...
                      {searchResults.subsubcategories.map(subsub => (
                        ...
                        >
                          <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-100">
                            {subsub.image && <img src={`${baseUrl}${subsub.image}`} alt={subsub.name} className="w-full h-full object-cover" />}
                          </div>
                          <span className="text-sm text-gray-700 dark:text-gray-200">{subsub.name}</span>
                        </button>
                      ))}
                    </div>
                  )}
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/menu" className="text-primary-500 text-sm font-semibold flex items-center gap-1 hover:text-primary-600">
          ...
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
            ...
            {catLoading ? (
              ...
              categories.map((cat) => (
                <button key={cat._id} onClick={() => { setActiveCategory(cat._id); navigate(`/menu?categoryId=${cat._id}`) }}
            ...
            <button onClick={() => { setCatSheetOpen(true); setActiveCategory('all'); }}
          ...
          <section>
            <div className="bg-gradient-to-r from-primary-500 to-orange-400 rounded-3xl p-5 flex items-center justify-between overflow-hidden relative">
              <div className="relative z-10">
                ...
                <Link to="/menu" className="mt-3 inline-flex items-center gap-1 bg-white text-primary-600 font-bold text-sm px-4 py-2 rounded-xl hover:shadow-md transition-all">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {combos.filter(c => c.status === 'active' || c.status === true).slice(0, 6).map(combo => (
                ...
                >
                  <div className="relative">
                    <img
                      src={combo.comboImage ? `${baseUrl}${combo.comboImage}` : '/placeholder-combo.png'}
                      alt={combo.comboName}
